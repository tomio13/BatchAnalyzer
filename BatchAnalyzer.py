...
def ReadConf(filename, default={}, simplify=False, strict=False, verbose=False):
    ...
    if os.path.isfile(filename):
        ...
        for i in alltxt:
            ...
            if "=" in i:
                ...
                key = txt[0].rstrip()
                val = txt[1].split('"')[1] if '"' in txt[1] else txt[1].strip()
                # val = txt[1].split()[0]
    ...
    **args,
    ...
    if len(y) > 0:
        if len(xerr) == 0 or len(yerr) == 0:
            plt1.plot(x, y, fmt, markersize=markersize, alpha=alpha, **args)

        else:
            plt1.errorbar(
                x, y, yerr, xerr, fmt=fmt, markersize=markersize, alpha=alpha, **args
            )
    ...
    if filename != "":
        ...
        pl.savefig(
            os.path.join(outpath, fn), dpi=dpi, bbox_inches="tight", pad_inches=0
        )
    # end if filename
    pl.draw()
